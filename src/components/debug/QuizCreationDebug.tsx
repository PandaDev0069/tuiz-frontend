...
// Custom hook for debug state management
const useDebugState = () => {
...
}> = ({ currentStep, isMinimized, onToggleMinimize, onMouseDown }) => {
  const stepIcons = {
    1: <FileText size={12} />,
    2: <Clock size={12} />,
    3: <Settings size={12} />,
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '10px 12px',
        borderBottom: !isMinimized ? '1px solid rgba(255, 255, 255, 0.2)' : 'none',
        cursor: 'grab',
...
// Quiz ID status component
const QuizIdStatus: React.FC<{ quizId: string | null }> = ({ quizId }) => (
  ...
  <div style={{ padding: '0 12px 8px', display: 'flex', gap: '4px' }}>
    ...
    >
      ...
      {debugEntriesCount > 0 && (
        ...
        >
          {debugEntriesCount > 9 ? '9+' : debugEntriesCount}
        </span>
      )}
    </button>
  </div>
...
// Form data details component
const FormDataDetails: React.FC<{
  formData: Record<string, unknown>;
  currentStep: number;
  quizId: string | null;
  pathname: string;
}> = ({ formData, currentStep, quizId, pathname }) => (
  <div style={{ padding: '8px 12px', borderTop: '1px solid rgba(255, 255, 255, 0.2)' }}>
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          navigator.clipboard.writeText(JSON.stringify({ currentStep, quizId, formData }, null, 2));
        }}
        onMouseDown={(e) => e.stopPropagation()}
        style={{
          flex: 1,
                    ...
                    >
                      {(() => {
                        try {
                          ...
                          return dataStr.length > 100 ? dataStr.slice(0, 100) + '...' : dataStr;
                        } catch {
...
export const QuizCreationDebug: React.FC<QuizCreationDebugProps> = ({
  ...
  const { position, isDragging, handleMouseDown } = useDrag({ x: 20, y: 20 });
  const {
    showDetails,
    setShowDetails,
    showActivityLog,
    ...
    >
      ...
      {!isMinimized && (
        <>
          ...
          <QuizIdStatus quizId={quizId} />
          <ToggleButtons
            showDetails={showDetails}
            showActivityLog={showActivityLog}
            onToggleDetails={() => setShowDetails(!showDetails)}
            onToggleActivityLog={() => setShowActivityLog(!showActivityLog)}
            debugEntriesCount={debugEntries.length}
          />
          ...
          {showActivityLog && (
            <ActivityLog debugEntries={debugEntries} onClearLog={() => debugLogger.clear()} />
          )}
        </>
      )}
